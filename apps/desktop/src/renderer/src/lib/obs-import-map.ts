...
// OBS import O2 — the mapping core (plan: vault "2026-07-07 - Zemara OBS
// Import Plan"). Pure: (ObsSetup, Device[]) → a Zemara config patch + the
...
const SKIP_NOTES: Partial<Record<ObsSource['kind'], string>> = {
  browser: 'Zemara has no browser source',
  text: 'Zemara has no text overlay source',
  media: 'Zemara has no media-file source',
  other: 'no Zemara equivalent'
}
...
export function mapObsSetup(setup: ObsSetup, devices: Device[]): ObsImportPlanResult {
  ...
  if (screenPick) {
    ...
    for (const extra of extras) {
      report.push({
        verdict: 'approximated',
        subject: extra.name,
        note: 'Zemara scenes hold one screen — kept the topmost'
      })
    }
  }
  ...
  if (cameraPick) {
    ...
    for (const extra of extras) {
      report.push({
        verdict: 'approximated',
        subject: extra.name,
        note: 'Zemara scenes hold one camera — kept the topmost'
      })
    }
  }
  ...
  if (imageItems.length > 0) {
    ...
    for (const item of imageItems.slice(1)) {
      report.push({
        ...
        note: 'Zemara supports one background image per scene'
      })
    }
  }
  ...
  if (setup.hasDesktopAudio) {
    report.push({
      ...
      note: 'system-audio capture is on the Zemara roadmap'
    })
  }
  ...
  for (const other of setup.scenes) {
    ...
    report.push({
      ...
      note: 'Zemara keeps one active scene — imported the current OBS scene; switch layouts in Studio'
    })
  }
