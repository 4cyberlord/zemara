...
export function Sidebar({
  ...
  return (
    <aside className="-mt-9 flex w-56 shrink-0 flex-col border-r bg-sidebar pt-9 text-sidebar-foreground backdrop-blur-2xl">
      ...
      {/* Five zones (ux-ia-refactor-plan): stage row, SETUP, CONTENT, MINISTRY, SYSTEM. */}
      <nav aria-label="Primary" className="flex flex-1 flex-col gap-5 overflow-y-auto px-3 py-2">
        ...
        <div className="flex flex-col gap-0.5">
          <GroupLabel>Ministry</GroupLabel>
          {tabsIn('ministry').map((tab) => (
            <NavRow
              key={tab.id}
              icon={tab.icon}
              label={tab.label}
      ...
      <div className="flex min-h-11 items-center justify-between gap-2 border-t px-3 py-1.5">
        {/* Zemara product-account control. Backend status is secondary (a small
