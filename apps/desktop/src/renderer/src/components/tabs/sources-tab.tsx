...
export function SourcesTab(): ReactElement {
  ...
  const capturePermissionTargetName =
    runtimeInfo?.capturePermissionTargetName ?? runtimeInfo?.permissionTargetName ?? 'Zemara'
  ...
  const applyCameraSource = (cameraId: string | undefined): void => {
    ...
    // A camera that only needs permission is selectable but isn't usable
    // yet, and nothing else in the app will ever attempt to open it unless
    // the current scene's layout happens to use a camera. Force the open
    // attempt right now instead — that's what triggers the real macOS
    // permission dialog.
    const selected = cameraId ? cameras.find((device) => device.id === cameraId) : undefined
    if (cameraId && selected?.status === 'permission-required') {
      void requestCameraPermission(cameraId)
      return
    }
    setCaptureConfig((current) => ({ ...current, sources }))
  }
            ...
            <AlertDescription className="flex flex-wrap gap-2 pt-2">
              {cameraPermissionAction ? (
                <Button
                  ...
                  variant="outline"
                  onClick={() => {
                    // Electron's own permission (what handleSystemPermission
                    // requests) is a different process identity than
                    // videorc-backend, which is what actually needs camera
                    // access — granting Electron's does nothing for it. Go
                    // straight at the backend with a known camera instead.
                    if (cameraPermissionAction === 'request-media-access' && cameras[0]) {
                      void requestCameraPermission(cameras[0].id)
                      return
                    }
                    void handleSystemPermission('camera')
                  }}
                >
