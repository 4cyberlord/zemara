...
// A cold source open (first camera/screen capture start in a session) does real
// device negotiation work — AVFoundation device power-on and format negotiation
// alone commonly cost 1-2s, ScreenCaptureKit stream setup is comparable — and 5s
// left no margin, so ordinary switches between scenes were tripping this as a
// hard failure rather than just being slow once.
const WARM_SOURCE_START_TIMEOUT: Duration = Duration::from_secs(12);
const WARM_SOURCE_POLL: Duration = Duration::from_millis(100);
const LAYOUT_INTENT_CANCEL_POLL: Duration = Duration::from_millis(25);
// A source not needed by the *current* scene is very often needed again a few
// scene switches later in the same editing/recording session (flipping between
// Camera and Screen + Cam while framing a shot, say). Retiring it almost
// immediately meant nearly every switch paid the full warm-path cold-start cost
// above instead of the hot path's instant swap (`classify_mutation` only grants
// Hot when the source is already live). Keeping both sources warm for several
// minutes of disuse makes ordinary switching hit the hot path; a source that
// really has gone unused this long still releases the device.
const UNUSED_SOURCE_STOP_GRACE: Duration = Duration::from_secs(300);
...
async fn retire_unused_sources_after_commit(
    ...
    if !needs.screen {
        // Grace period, not immediate: see UNUSED_SOURCE_STOP_GRACE. Detached so
        // its potentially slow join never blocks the layout-apply response.
        let grace_state = state.clone();
        tokio::spawn(async move {
            sleep(UNUSED_SOURCE_STOP_GRACE).await;
            let stop = {
                let intents = grace_state.layout_intents.lock().await;
                if intents.latest_intent_id == intent_id && !intents.latest_needs_screen {
                    Some(begin_preview_screen_stop(&grace_state).await)
                } else {
                    None
                }
            };
