...
// A first-time permission request pauses on a system dialog for the user to
// answer — the old 4s budget was sized only for opening an already-authorized
// device and gave up on the permission flow long before a person could react.
const MACOS_CAMERA_PREVIEW_STARTUP_TIMEOUT: Duration = Duration::from_secs(45);

fn native_camera_preview_thread_startup_timeout() -> Duration {
    if cfg!(target_os = "windows") {
        WINDOWS_CAMERA_PREVIEW_STARTUP_TIMEOUT
    } else {
        MACOS_CAMERA_PREVIEW_STARTUP_TIMEOUT
    }
}
...
mod macos {
    ...
    // AVFoundation only shows the system consent dialog once a process
    // actually asks for camera access. A `NotDetermined` status previously
    // made `start_session` bail out immediately, so a non-bundled dev binary
    // like this one could never be prompted — nothing in the app ever called
    // the API that triggers the dialog. This explicitly requests access and
    // blocks (safe: this runs on the dedicated camera-preview thread, not the
    // Electron/main thread) until the user answers, or until AVFoundation
    // resolves immediately for an already-decided status.
    fn request_camera_access_and_wait() {
        let Some(video_media_type) = (unsafe { AVMediaTypeVideo }) else {
            tracing::warn!("request_camera_access_and_wait: AVMediaTypeVideo unavailable");
            return;
        };
        tracing::info!("request_camera_access_and_wait: calling requestAccessForMediaType");
        ...
        match rx.recv_timeout(MACOS_CAMERA_PREVIEW_STARTUP_TIMEOUT) {
            Ok(granted) => tracing::info!(granted, "request_camera_access_and_wait: completion handler fired"),
            Err(error) => tracing::warn!(%error, "request_camera_access_and_wait: timed out or channel closed without a result"),
        }
    }
    ...
    fn start_session(
        ...
        let mut permission = native_camera_permission();
        tracing::info!(?permission, camera_id = %config.camera_id, "start_session: initial camera permission");
        if permission == NativeCameraPermission::NotDetermined {
            request_camera_access_and_wait();
            permission = native_camera_permission();
            tracing::info!(?permission, "start_session: camera permission after request");
        }
        if permission != NativeCameraPermission::Authorized {
            tracing::warn!(?permission, "start_session: camera not authorized, aborting");
