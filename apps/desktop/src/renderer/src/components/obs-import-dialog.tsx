...
// OBS import O4 (plan: vault "2026-07-07 - Zemara OBS Import Plan"): pick
...
// product: it explains Zemara in terms of the user's own OBS setup.
        ...
        {discovery?.available ? (
          <div className="flex flex-col gap-3">
            <div className="grid gap-3 sm:grid-cols-2">
              <AppSelect
                aria-label="OBS scene collection"
                disabled={applying}
                placeholder="Scene collection"
                value={collection}
                onValueChange={(value) => {
                  invalidateSetup()
                  setCollection(value)
                }}
                options={discovery.collections.map((name) => ({ value: name, label: name }))}
              />
              <AppSelect
                aria-label="OBS profile"
                disabled={applying}
                placeholder="Profile"
                value={profile}
                onValueChange={(value) => {
                ...
                options={discovery.profiles.map((name) => ({ value: name, label: name }))}
              />
            </div>
