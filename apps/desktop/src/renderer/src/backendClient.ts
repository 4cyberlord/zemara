...
const METHOD_REQUEST_TIMEOUT_MS: Readonly<Record<string, number>> = {
  ...
  // A cold camera device start (first open after a fresh permission grant)
  // can briefly hold the same backend locks diagnostics.stats reads,
  // pushing an otherwise-instant call past a tight budget. 20s tolerates
  // that one-time startup collision without masking a genuinely stuck backend.
  'diagnostics.stats': 20_000,
  'devices.list': 30_000,
