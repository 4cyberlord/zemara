...
describe('renderer security policy', () => {
  ...
  it('applies a restrictive CSP to every bundled renderer and a nonce to Notes', () => {
    ...
    // img-src and media-src both allow https: (2026-07-16): Sermons/Videos
    // thumbnails and playback sources are admin-authored URLs from the
    // operator's own backend (YouTube thumbnails, Supabase storage, etc.),
    // not arbitrary third-party page content — same trust tier as the
    // videorc.com connect-src allowance above.
    expect(
      RENDERER_DOCUMENT_CSP.split('; ').find((directive) => directive.startsWith('img-src'))
    ).toContain('https:')
    expect(
      RENDERER_DOCUMENT_CSP.split('; ').find((directive) => directive.startsWith('media-src'))
    ).toContain('https:')
    expect(source('../../electron.vite.config.ts')).toContain(
