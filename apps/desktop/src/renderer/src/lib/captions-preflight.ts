...
export function decideGoLiveCaptionsReadiness(input: {
  ...
  if (!captions) {
    return blocked(
      'Zemara could not verify live-caption readiness for this deployment. Try again, or continue this session without captions.'
    )
  }
...
function unavailableReason(
  ...
  switch (reasonCode) {
    ...
    case 'captions-disabled':
      return 'Live captions are temporarily disabled on this Zemara deployment.'
    case 'captions-invalid-config':
    case 'captions-not-configured':
      return 'Live captions are not configured correctly on this Zemara deployment.'
    default:
      return 'Live captions are not ready on this Zemara deployment.'
  }
}
