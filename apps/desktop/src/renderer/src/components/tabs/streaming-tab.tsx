        ...
        <span onClick={(event) => event.stopPropagation()}>
          {enableLockGate ? (
            ...
            <button
              ...
              aria-label={`${target.label} requires Zemara Premium`}
      ...
      <>
        ...
        {platform === 'youtube' ? (
          <Dialog open={youtubeConsentOpen} onOpenChange={setYoutubeConsentOpen}>
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Connect YouTube to Zemara</DialogTitle>
                <DialogDescription>
                  Zemara uses YouTube API Services to identify your channel and manage your live
              ...
              <div className="grid gap-3 text-sm">
                ...
                <label className="flex items-start gap-3 rounded-row border bg-muted/30 p-3">
                  ...
                  />
                  <span>
                    I agree to the Zemara and YouTube terms and want to connect my YouTube account.
                  </span>
                </label>
              </div>
              <DialogFooter>
        ...
        <Field>
          ...
          <div className="flex gap-2">
            <AppSelect
              ...
              options={youtubeChannelOptions.map((channel) => ({
                value: channel.channelId,
                label: `${channel.title}${channel.handle ? ` (${channel.handle})` : ''}`
              }))}
            />
            <Button
          ...
          <Field>
            ...
            {twitchCategoryOptions.length ? (
              <AppSelect
                ...
                options={twitchCategoryOptions.map((category) => ({
                  value: category.id,
                  label: category.name
                }))}
              />
