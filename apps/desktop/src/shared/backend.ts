...
// The desktop's Zemara PRODUCT account (mirrors the Rust VideorcAccountSnapshot).
...
// The admin backend connection (Settings > Database): a self-hosted
// prophet-roja-admin deployment Zemara reads/writes Sermons through. No
// database credentials ever live in Zemara — only this base URL + API key,
// stored via the local secret store (mirrors the Rust AdminConnection).
export interface AdminConnection {
  baseUrl: string | null
  apiKeyConfigured: boolean
  /** Last 4 characters of the stored key, e.g. "…OkI0" — never the full key. */
...
// admin API's own JSON, which always includes nullable columns as `null`
// rather than omitting the key. Fields below are `| null`, not optional.
export interface AdminSermonSeries {
  id: string
  title: string
  slug: string | null
...
export interface AdminSermon {
  ...
  createdAt: string
  updatedAt: string
  series: AdminSermonSeries | null
  topics: string[]
}

// PATCH semantics on the admin API: omit a field to leave it unchanged, or
// send "" to clear a nullable one (the admin API's repository already treats
// "" as "set to null").
export interface AdminSermonInput {
  title?: string
  seriesId?: string
  speakerName?: string
  scripture?: string
...
// Living Text — Bible verse citation detection. Runs entirely locally
// (crates/videorc-backend/src/living_text.rs, a local SQLite cache synced
// once from the admin API); these RPC calls never round-trip per detection.
export interface LivingTextBook {
  bookNumber: number
  name: string
}

/** A translation available to sync from the admin API, but not necessarily cached yet. */
export interface LivingTextAvailableTranslation {
  id: string
  code: string
  name: string
  language: string
...
/** A translation already synced into the local cache. */
export interface LivingTextTranslation {
  code: string
  name: string
  language: string
  verseCount: number
...
export interface LivingTextVerse {
  book: string
  bookNumber: number
  chapter: number
  verse: number
...
export interface LivingTextSyncSummary {
  code: string
  name: string
  verseCount: number
}

export interface LivingTextCitation {
  book: string
  bookNumber: number
  chapter: number
  verseStart: number | null
  verseEnd: number | null
  matchType: 'explicit' | 'chapter'
  sourceText: string
  index: number
}

export interface LivingTextResolvedCitation extends LivingTextCitation {
  resolved: boolean
  verses: LivingTextVerse[]
}

// Rust's AdminVideo/VideoCategory serialize every Option<T> as an explicit
// JSON `null` (no skip_serializing_if on these result types) — same
// convention as AdminSermon above. Nullable, not optional/undefined.
...
// PATCH semantics on the admin API: omit a field to leave it unchanged (no
// "clear to empty" convention here — videoUrl/speakerName aren't cleared via
// admin.videos.update the way sermon fields are).
...
// JSON `null` (no skip_serializing_if on these result types) — same
// convention as AdminSermon/AdminVideo above. Nullable, not
// optional/undefined. Sensitive fields the admin API's row happens to
// include (passwordHash, passwordResetToken, clerkUserId) are intentionally
// absent here — the Rust struct never parses them.
...
export interface AdminMember {
  ...
  /** Only non-null in the response right after admin.members.create — see
   *  Rust's AdminMember doc comment. Never re-fetchable afterward. */
  defaultPassword: string | null
}

// PATCH semantics on the admin API: omit a field to leave it unchanged.
...
export interface MembersQuery {
  query?: string
  status?: 'active' | 'inactive' | 'push' | 'all'
  country?: string
  page?: number
  pageSize?: number
}
...
// JSON `null` (no skip_serializing_if on this result type) — same
// nullable-not-optional convention as AdminSermon/AdminVideo/AdminMember.
...
export interface CreateCampaignInput {
  ...
  body: string
  target?: string
  /** ISO 8601; presence (in the future) is what makes the admin API
...
// PATCH semantics — only draft/scheduled campaigns can be updated.
...
// (no skip_serializing_if on this result type) — same nullable-not-optional
// convention as AdminSermon/AdminVideo/AdminMember/NotificationCampaign.
export interface AdminEvent {
  id: string
  /** Which church branch/campus this belongs to — distinct from
   *  locationName/locationAddress/locationLat/locationLng below, which are
   *  the venue's own address, not the owning branch. */
  churchLocationId: string | null
  title: string
  slug: string
  description: string | null
  category: string
  /** "In-Person" | "Live Stream" | "Hybrid" */
  eventType: string
  ...
  isRecurring: boolean
  /** JSON string: {freq: 'weekly' | 'monthly', interval: number, until: string | null} */
...
export interface CreateEventInput {
  ...
  /** ISO 8601; series stops generating occurrences after this date */
...
export interface UpdateEventInput {
  ...
  imagePath?: string
  capacity?: number
  /** Applies this edit (except startsAt/endsAt) to every future occurrence in the series. */
  applyToSeries?: boolean
}

// QR-code attendance — one AttendanceSession per Event occurrence, created
// on demand when an admin opens attendance. No session for an event means
// attendance was never opened for that occurrence.
...
export interface AttendanceRecord {
  ...
  distanceFromEventMeters: number | null
...
/** Minted fresh on every call — no per-rotation row is persisted server-side. */
...
/** Fixed set — no per-church configurable type table for now. */
...
export interface UnitMemberSummary {
  id: string
  fullName: string
  avatarUrl: string | null
  phone?: string | null
...
// nestable type (via parentUnitId) — distinguished only by `type`. List
// responses include `positions`/memberCount; the single-unit detail response
// also includes `memberships`/`childUnits`/`parentUnit`.
export interface OrganizationalUnit {
  ...
  positions?: OrganizationalUnitPosition[]
  memberships?: OrganizationalUnitMembership[]
...
export interface OrganizationalUnitMembership {
  ...
  createdAt: string
  updatedAt: string
  member?: UnitMemberSummary
}

export interface OrganizationalUnitPosition {
  id: string
  unitId: string
  name: string
  ...
  createdAt: string
  updatedAt: string
  assignments?: OrganizationalUnitPositionAssignment[]
}

// History of who held a position and when, rather than a single leaderId on
// the unit — a past Choir Director gets status "ended" + endedAt instead of
// just disappearing when a new one is assigned.
export interface OrganizationalUnitPositionAssignment {
  id: string
  unitId: string
  positionId: string
  memberId: string
...
export interface CreateUnitPositionInput {
  name: string
  isLeadershipPosition?: boolean
}
...
/** A person with dashboard access — never includes a password hash. */
...
export interface UploadImageInput {
  /** One of "sermons" | "events" | "announcements" | "series" — the admin API rejects anything else. */
  folder: string
  fileName: string
  mimeType: string
...
export interface RuntimeInfo {
  ...
  /** True when Zemara is running with hardware acceleration disabled — via
...
// Living Text broadcast output — a plain, undecorated window rendering the
// live verse full-bleed so external capture software (OBS Window Capture,
// Zoom, etc.) can pick it up with no plugins or network setup. Deliberately
// NOT content-protected (the opposite of the Notes window): this window is
// meant to be captured.
export interface LivingTextOutputState {
  open: boolean
}

export interface LivingTextOutputTheme {
...
export interface LivingTextOutputVerse {
  text: string
  reference: string
}

export interface LivingTextOutputPayload {
  theme: LivingTextOutputTheme
  verse: LivingTextOutputVerse | null
}
...
export interface VideorcApi {
  ...
  /** Opens a native Save dialog and writes the given text content (e.g. a
   *  CSV export) to the chosen path. Null path means the user cancelled. */
  exportTextFile: (
    content: string,
  ...
  openLivingTextOutput: () => Promise<LivingTextOutputState>
  closeLivingTextOutput: () => Promise<LivingTextOutputState>
  getLivingTextOutputState: () => Promise<LivingTextOutputState>
  pushLivingTextOutput: (payload: LivingTextOutputPayload) => Promise<void>
  onLivingTextOutputState: (callback: (state: LivingTextOutputState) => void) => () => void
  onLivingTextOutputUpdate: (callback: (payload: LivingTextOutputPayload) => void) => () => void
  ...
  // Quits the app outright — window-all-closed does not quit on macOS, so
  // this is the only way a renderer action (e.g. declining the terms gate)
  // can actually terminate the app there.
  quitApp: () => Promise<void>
  // Shrinks the window to a small centered panel for first-run gates
  // (terms, welcome what's-new) and restores the normal workspace size
  // once they clear — so the real app never renders inside an oversized,
  // mostly-empty window behind a small dialog.
  setWindowGated: (gated: boolean) => Promise<void>
...
// "2026-06-06 - Zemara In-App Livestream Comments Plan". The backend owns
...
/** One OBS input source, reduced to the kinds Zemara can reason about. */
export interface ObsSource {
  name: string
  /** Zemara-facing classification of the OBS source id. */
  kind:
    | 'display'
    | 'window'
    | 'application'
