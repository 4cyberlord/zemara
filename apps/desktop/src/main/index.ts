...
import type {
  ...
  LayoutSettings,
  LiveChatSnapshot,
  LivingTextOutputPayload,
  LivingTextOutputState,
...
// Sane default so a freshly opened output window (before the Living Text tab
// has pushed anything this session) shows a calm placeholder instead of a
// blank window an operator might assume is broken.
const DEFAULT_LIVING_TEXT_OUTPUT_PAYLOAD: LivingTextOutputPayload = {
  theme: {
...
let livingTextOutputWindow: BrowserWindow | null = null
let livingTextOutputPayload: LivingTextOutputPayload = DEFAULT_LIVING_TEXT_OUTPUT_PAYLOAD
...
let youtubeBridgeServer: HttpServer | null = null
let youtubeBridgeServerPort: Promise<number> | null = null
...
app.on('child-process-gone', (_event, details) => {
  ...
  logBackend(
    'warn',
    `GPU process crashed (${details.reason}, ${gpuProcessCrashCount} this launch). Repeated crashes switch Zemara to software rendering on the next launch.`
  )
  if (!gpuFallbackDecision.disable && !gpuFallbackPersistedThisLaunch) {
    if (shouldPersistGpuFallback(gpuProcessCrashCount)) {
      ...
      logBackend(
        'warn',
        'GPU process is unreliable on this machine — Zemara will use software rendering from the next launch (set VIDEORC_FORCE_GPU=1 to undo).'
      )
    }
  }
})
...
// Base launch size is 1180x780; tripled per request, but clamped to the
// current display's work area so it can't open larger than the screen on
// laptops/smaller monitors.
function computeLaunchWindowSize(): { width: number; height: number } {
  const workArea = screen.getPrimaryDisplay().workAreaSize
  return {
    width: Math.min(3540, workArea.width),
    height: Math.min(2340, workArea.height)
  }
}

// The compact panel size used while a first-run gate (terms, welcome
// what's-new) is showing — sized to comfortably fit those dialogs without
// the enormous mostly-empty workspace window behind them, matching how a
// native installer-style license panel actually looks (Xcode, Icon
// Composer): a small standalone window, not a full-size app window with a
// dialog floating in it.
const GATE_WINDOW_SIZE = { width: 640, height: 760 } as const

function createWindow(): void {
  const { width: launchWidth, height: launchHeight } = computeLaunchWindowSize()
  ...
  mainWindow.webContents.on('before-input-event', (event, input) => {
    ...
    // ⌘R/⌘I/⌘E/⌘B/⌘L/⌘U/⌘T: Sermons/Videos/Events/Members/Alerts/Publish/
    // Living Text — every plain ⌘1–⌘9 slot is already spoken for by Studio,
    // the six Setup panels, and Library, so these get single-letter
    // mnemonics instead of a second ⌘⇧-digit tier. Letters were picked to
    // avoid macOS/app-standard meanings (⌘V paste, ⌘M minimize, ⌘S save,
    // ⌘N new, ⌘K search, ⌘P preview are all already spoken for or reserved)
    // — Sermons/Videos took an interior letter instead of their initial for
    // that reason.
    const navLetter = input.key.toUpperCase()
    if (['R', 'I', 'E', 'B', 'L', 'U', 'T'].includes(navLetter)) {
      event.preventDefault()
      if (mainWindow) {
        sendElectronEvent(mainWindow.webContents, 'shortcut:navigate', navLetter)
      }
    }
  })
  ...
  mainWindow.on('closed', () => {
    ...
    if (livingTextOutputWindow && !livingTextOutputWindow.isDestroyed()) {
      livingTextOutputWindow.close()
    }
...
function notesWindowHtml(document: NotesDocument): string {
  ...
  return `<!doctype html><html><head><meta charset="utf-8">
    ...
    <div class="drag-bar"><span class="title">Zemara Notes</span><span class="spacer"></span>
...
// --- Living Text broadcast output window --------------------------------------
// A plain, undecorated window rendering the live verse full-bleed so external
// capture software (OBS Window Capture, Zoom, Teams, anything) can pick it up
// with zero plugins or network configuration — point a Window Capture source
// at it and go. Deliberately NOT content-protected (the opposite of Notes):
// this window exists to be captured. It never invokes anything itself; the
// main renderer's Living Text tab owns open/close/push, and this window just
// renders whatever payload it's handed.
function livingTextOutputState(): LivingTextOutputState {
  return { open: Boolean(livingTextOutputWindow && !livingTextOutputWindow.isDestroyed()) }
}

function emitLivingTextOutputState(): void {
  if (mainWindow && !mainWindow.isDestroyed()) {
    sendElectronEvent(mainWindow.webContents, 'livingText-output:state', livingTextOutputState())
  }
}

function livingTextOutputWindowHtml(payload: LivingTextOutputPayload): string {
  const scriptNonce = randomBytes(24).toString('base64url')
  const contentSecurityPolicy = inlineRendererDocumentCsp(scriptNonce)
  const initialPayloadJson = jsonForInlineScript(payload)
  return `<!doctype html><html><head><meta charset="utf-8">
    <meta http-equiv="Content-Security-Policy" content="${contentSecurityPolicy}">
    <style>
    html, body { margin: 0; height: 100%; overflow: hidden; background: #000;
      font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif; }
    body { display: flex; flex-direction: column; }
    .drag-bar { height: 28px; flex-shrink: 0; display: flex; align-items: center;
      gap: 8px; padding: 0 10px 0 74px; box-sizing: border-box;
      background: rgba(20, 20, 20, 0.9); border-bottom: 1px solid rgba(255,255,255,0.08);
      -webkit-app-region: drag; }
    .drag-bar .title { color: rgba(255,255,255,0.55); font-size: 10.5px;
      letter-spacing: .08em; text-transform: uppercase; }
    .stage { flex: 1; display: flex; align-items: center; justify-content: center;
      padding: 6% 8%; box-sizing: border-box; text-align: center; }
    .verse-text { font-weight: 600; line-height: 1.35; white-space: pre-wrap;
      text-wrap: balance; }
    .verse-reference { margin-top: 0.6em; font-weight: 500; letter-spacing: .02em;
      opacity: 0.9; }
    .placeholder { color: rgba(255,255,255,0.3); font-size: 15px; }
  </style></head><body>
    <div class="drag-bar"><span class="title">Bible Verse Output</span></div>
    ...
    <script nonce="${scriptNonce}">
      (() => {
        ...
        function render(payload) {
          ...
          stage.style.textAlign = theme.textAlign;
          stage.style.justifyContent =
            theme.textAlign === 'left' ? 'flex-start' : theme.textAlign === 'right' ? 'flex-end' : 'center';
          if (!payload.verse) {
            stage.innerHTML = '';
            const placeholder = document.createElement('div');
            placeholder.className = 'placeholder';
            placeholder.textContent = 'Bible Verse — waiting for a verse…';
          ...
          verseEl.style.color = theme.textColor;
          verseEl.style.fontSize = theme.fontSize + 'px';
          verseEl.textContent = payload.verse.text;
          const refEl = document.createElement('div');
          refEl.className = 'verse-reference';
          refEl.style.color = theme.referenceColor;
          refEl.style.fontSize = Math.round(theme.fontSize * 0.4) + 'px';
          refEl.textContent = payload.verse.reference;
          stage.appendChild(verseEl);
          stage.appendChild(refEl);
        }

        render(${initialPayloadJson});
        if (window.videorc && window.videorc.onLivingTextOutputUpdate) {
          window.videorc.onLivingTextOutputUpdate(render);
        }
      })();
    </script>
  </body></html>`
...
async function openLivingTextOutputWindow(): Promise<LivingTextOutputState> {
  const existingWindow = livingTextOutputWindow
  if (existingWindow && !existingWindow.isDestroyed()) {
    existingWindow.show()
    existingWindow.focus()
    return livingTextOutputState()
  }
  ...
  const window = new BrowserWindow({
    ...
    minWidth: 480,
    minHeight: 270,
    title: 'Bible Verse Output',
    ...(isMac
      ? { titleBarStyle: 'hiddenInset' as const, trafficLightPosition: { x: 14, y: 8 } }
      : {}),
    backgroundColor: livingTextOutputPayload.theme.backgroundColor,
    show: false,
    ...appWindowIconOptions(),
    webPreferences: {
      preload: join(__dirname, '../preload/index.js'),
      ...rendererWindowWebPreferences('livingTextOutput'),
      backgroundThrottling: false
    }
  })
  registerRendererWindow(window, 'livingTextOutput')
  attachAuxWindowShortcuts(window)
  livingTextOutputWindow = window

  window.on('closed', () => {
    if (livingTextOutputWindow === window) {
      livingTextOutputWindow = null
      emitLivingTextOutputState()
    }
  })

  const outputDocumentUrl = `data:text/html;charset=utf-8,${encodeURIComponent(
    livingTextOutputWindowHtml(livingTextOutputPayload)
  )}`
  trustRendererDocument(window, outputDocumentUrl)
  await window.loadURL(outputDocumentUrl)
  window.show()
  emitLivingTextOutputState()
  return livingTextOutputState()
}

function closeLivingTextOutputWindow(): LivingTextOutputState {
  if (livingTextOutputWindow && !livingTextOutputWindow.isDestroyed()) {
    livingTextOutputWindow.close()
  }
  return livingTextOutputState()
...
function pushLivingTextOutputPayload(payload: LivingTextOutputPayload): void {
  livingTextOutputPayload = payload
  if (livingTextOutputWindow && !livingTextOutputWindow.isDestroyed()) {
    sendElectronEvent(livingTextOutputWindow.webContents, 'livingText-output:update', payload)
  }
}
...
const PREVIEW_WINDOW_HTML = `<!doctype html><html><head><meta charset="utf-8"><style>
  ...
  <div class="drag-bar"><span class="label">Zemara Preview</span><span class="grip"></span></div>
</body></html>`
...
function resolveAppIconPaths(): string[] {
  ...
  const candidates = app.isPackaged
    ? [
        join(process.resourcesPath, 'zemara-logo.png'),
        join(process.resourcesPath, 'icon.icns'),
        join(process.resourcesPath, 'icon.png')
      ]
    : [
        resolve(root, 'apps/desktop/src/renderer/src/assets/zemara-logo.png'),
...
// Zemara targets Windows 11 only (build 22000+): it unlocks Mica/acrylic,
...
function enforceWindowsVersionFloor(): boolean {
  ...
  dialog.showMessageBoxSync({
    ...
    message: 'Zemara requires Windows 11 or newer.',
    detail: `This machine reports Windows build ${release()}. Zemara needs build 22000 (Windows 11) or later.`,
    buttons: ['Quit']
  })
  return true
}
...
// `cargo run` links and execs the backend in one step, giving no window to
// codesign in between — any signature applied to the binary afterward is
// gone by the next launch. So dev mode instead does `cargo build` (fast,
// usually sub-second on an unchanged tree) then signs the resulting binary
// itself before spawning it directly, skipping `cargo run` for the actual
// launch. Without a real entitlements-carrying signature, macOS TCC denies
// Camera/Mic to this process silently — no prompt, no error. Only runs when
// VIDEORC_DEV_CODESIGN_IDENTITY is set (a developer's own local cert name);
// no-op otherwise, so other contributors' dev builds are unaffected.
function devCodesignIdentity(): string | null {
  const identity = process.env.VIDEORC_DEV_CODESIGN_IDENTITY
  return identity && identity.trim().length > 0 ? identity.trim() : null
}

function buildAndSignDevBackend(root: string): string {
  const devBinary = resolveDevBackendBinary(root)
  const cargoBin = resolveCargoBinary()
  const buildResult = spawnSync(
    cargoBin,
    ['build', '--quiet', '-p', 'videorc-backend', '--bin', 'videorc-backend'],
    {
      cwd: root,
      env: { ...process.env, ...devCargoEnvOverrides() },
      stdio: 'inherit'
    }
  )
  if (buildResult.status !== 0) {
    logBackend('error', 'cargo build failed for videorc-backend; see output above.')
    return devBinary
  }

  const identity = devCodesignIdentity()
  ...
  if (signResult.status !== 0) {
    logBackend(
      'warn',
      `Dev codesign of videorc-backend failed: ${signResult.stderr?.toString().trim() ?? signResult.status}`
    )
  } else {
    logBackend('info', `Dev-signed videorc-backend with identity: ${identity}`)
  }
  return devBinary
}
...
// Single-backend policy: reap only children a previous Zemara launch recorded.
...
function scheduleBackendRestart(code: number | null, signal: NodeJS.Signals | null): void {
  ...
  if (attempt > BACKEND_RESTART_BACKOFF_MS.length) {
    logBackend(
      'error',
      'Backend crashed repeatedly; automatic restarts stopped. Restart Zemara to recover.'
    )
...
function startBackendWithRegistryLock(): void {
  ...
  const cargoBinDir = join(homedir(), '.cargo', 'bin')
  const ffmpegBinDir = resolvePackagedFfmpegBinDir()
  const command = app.isPackaged ? resolvePackagedBackendBinary() : buildAndSignDevBackend(root)
  const args: string[] = []
...
async function primeScreenCapturePermission(): Promise<void> {
  ...
  try {
    ...
    logBackend(
      'warn',
      `Screen Recording permission is ${status} for ${info.permissionTargetName}; capture runs in ${info.capturePermissionTargetName}. Open Screen Recording settings, grant access, then quit and relaunch Zemara.`
    )
  } catch (error) {
...
// A YouTube video ID is normally 11 characters, but the bound is loose
// (matches YT's own documented charset) rather than hard-coding a length.
const YOUTUBE_VIDEO_ID_PATTERN = /^[A-Za-z0-9_-]{1,32}$/

/**
 * Serves a tiny static HTML page over http://127.0.0.1 (loopback only) whose
 * sole job is to run the real YouTube IFrame Player API and relay
 * play/pause/mute/volume/seek to/from the app's own file:// document via
 * postMessage. The YT API computes its origin handshake from
 * document.location; a file:// document's opaque origin makes YouTube reject
 * the embed outright ("Error 153"), so the player has to live on a real http
 * origin instead of inside the app's own document.
 */
function youtubeBridgeHtml(videoId: string): string {
  // JSON.stringify safely escapes the value as a JS string literal, but the
  // HTML parser still scans literally for "</script" inside that literal
  // regardless of JS string-quoting — escape the slash so a crafted ID can't
  // break out of the script tag.
  const safeVideoId = JSON.stringify(videoId).replace(/</g, '\\u003C')
  return `<!doctype html>
<html>
<head>
<meta charset="utf-8" />
...
(function () {
  ...
  function postState(extra) {
    ...
    try {
      window.parent.postMessage(Object.assign({
        ...
        type: 'state',
        muted: typeof player.isMuted === 'function' ? player.isMuted() : undefined,
        volume: typeof player.getVolume === 'function' ? player.getVolume() / 100 : undefined
      }, extra), '*');
    } catch (err) {}
  }
  ...
  window.onYouTubeIframeAPIReady = function () {
    player = new YT.Player('player', {
      ...
      events: {
        onReady: function (event) {
          ...
          postState({ duration: event.target.getDuration() });
        },
    ...
    setInterval(function () { postState({}); }, 400);
  };

  window.addEventListener('message', function (event) {
    var data = event.data;
    if (!data || data.source !== 'videorc-youtube-bridge' || data.type !== 'command' || !player) return;
    var fn = player[data.func];
    if (typeof fn === 'function') fn.apply(player, data.args || []);
  });
})();
</script>
</body>
...
function handleYoutubeBridgeRequest(request: IncomingMessage, response: HttpResponse): void {
  ...
  response.end(youtubeBridgeHtml(videoId))
}

function ensureYoutubeBridgeServer(): Promise<number> {
  youtubeBridgeServerPort ??= new Promise<number>((resolve, reject) => {
    const server = createServer((request, response) => {
      handleYoutubeBridgeRequest(request, response)
    })
    server.on('error', (error: unknown) => {
      youtubeBridgeServerPort = null
    ...
    server.listen(0, '127.0.0.1', () => {
      ...
      if (!address || typeof address === 'string') {
        youtubeBridgeServerPort = null
        reject(new Error('Failed to determine YouTube bridge server port.'))
        return
      }
      youtubeBridgeServer = server
      resolve(address.port)
    })
  })
  return youtubeBridgeServerPort
}
...
async function stopBackend(): Promise<void> {
  ...
  youtubeBridgeServer?.close()
  youtubeBridgeServer = null
  youtubeBridgeServerPort = null
...
// Native Save dialog + write-to-disk for renderer-generated exports (e.g. the
// Members CSV export) — the renderer builds the file content, the main
// process is the only side allowed to touch the filesystem.
...
if (!hasSingleInstanceLock) {
  // Correct but previously SILENT: `pnpm dev` shares the "Videorc" profile
  // (app.setName is intentionally unchanged by the Zemara rebrand, to keep
  // existing local data) with the packaged app, so a running
  // /Applications/Zemara.app holds this lock and dev exited without a word
  // (reported 2026-07-06). Say why we quit.
  console.error(
    'Zemara is already running (another instance holds the single-instance lock), ' +
      'so this instance is exiting and the running app was focused instead. ' +
      'For `pnpm dev`, quit the installed Zemara app first — or isolate this run ' +
...
app.whenReady().then(async () => {
  ...
  secureIpcHandle('system:export-text-file', (_event, content: string, suggestedFileName: string) =>
    exportTextFile(content, suggestedFileName)
  )
  ...
  secureIpcHandle('youtube-bridge:get-embed-url', async (_event, videoId: string) => {
    const port = await ensureYoutubeBridgeServer()
    return `http://127.0.0.1:${port}/embed?videoId=${encodeURIComponent(videoId)}`
  })
  ...
  // User-initiated quit from the renderer (e.g. declining the first-run terms
  // gate) — window-all-closed deliberately does NOT quit on macOS, so this is
  // the only way a renderer action can actually terminate the app there.
  secureIpcHandle('app:quit', () => {
    app.quit()
  })
  // Shrinks the window to a small centered panel while a first-run gate
  // (terms, welcome what's-new) is showing, and restores it to the normal
  // workspace size once the real app is ready to appear — see GATE_WINDOW_SIZE.
  secureIpcHandle('app:set-window-gated', (_event, gated: boolean) => {
    if (!mainWindow || mainWindow.isDestroyed()) return
    const { width, height } = gated ? GATE_WINDOW_SIZE : computeLaunchWindowSize()
  ...
  secureIpcHandle('livingText-output:open', () => openLivingTextOutputWindow())
  secureIpcHandle('livingText-output:close', () => closeLivingTextOutputWindow())
  secureIpcHandle('livingText-output:get-state', () => livingTextOutputState())
  secureIpcHandle('livingText-output:push', (_event, payload: LivingTextOutputPayload) => {
    pushLivingTextOutputPayload(payload)
    return undefined
  })
