...
import { AdminDatabaseSection } from '@/components/admin-database-section'
import { AdminIntegrationsSection } from '@/components/admin-integrations-section'
import { AdminLocationsSection } from '@/components/admin-locations-section'
  ...
  return (
    <div className="flex flex-col gap-5">
      <ConfigGrid>
          ...
          <FieldGroup>
            <Field>
              ...
              <div className="flex gap-2">
                ...
                >
                  {outputDirectory || 'Zemara default recordings folder'}
                </div>
            ...
            <div className="flex flex-col gap-2 rounded-row border border-warning/40 bg-warning/10 p-3">
              ...
              <p className="text-xs text-muted-foreground">
                {import.meta.env.DEV
                  ...
                  : 'FFmpeg ships with Zemara, so this usually means the install is damaged. Reinstall Zemara.'}
              </p>
            </div>
          ) : (
            ...
            <CollapsibleContent className="flex flex-col gap-3 pt-2">
              <div className="flex items-center gap-2 rounded-row border bg-muted/40 px-3 py-2 text-xs">
                ...
                <span className="min-w-0 flex-1 truncate text-muted-foreground">
                  Managed privately in Zemara app data
                </span>
              </div>
            </CollapsibleContent>
          </Collapsible>
        ...
        <PanelSection
          description={`What ${osSettingsName(runtimeInfo?.platform)} lets Zemara capture right now.`}
          icon={LockKey}
          title="System access"
          action={
      ...
      <div className="grid items-start gap-5 lg:grid-cols-2">
        <div className="flex flex-col gap-5">
          <PanelSection
            description="How Zemara looks and behaves on this device."
            icon={PaintBrush}
        ...
        <PanelSection
          description="Every keyboard shortcut in Zemara."
          icon={Keyboard}
          title="Shortcuts"
        >
...
function AboutAndUpdates({ onShowWhatsNew }: { onShowWhatsNew: () => void }): ReactElement {
  ...
  return (
    <PanelSection
      description="Check for new versions of Zemara and install them."
      icon={Sparkle}
      title="About & updates"
    >
    ...
    case 'downloaded':
      return (
        <div className="flex flex-col gap-2">
          ...
          <p className="text-xs text-muted-foreground">
            {captureActive
              ? 'Finish your recording first — installing restarts Zemara.'
              : 'Zemara will restart to finish updating.'}
          </p>
        </div>
      )
    case 'not-available':
