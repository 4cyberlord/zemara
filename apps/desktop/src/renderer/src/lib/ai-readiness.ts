...
export function cloudAiReadiness({
  ...
  if (loading && !capabilities) {
    return disabled('checking', 'Checking cloud AI', 'Checking Zemara AI readiness.')
  }
  ...
  if (!capabilities.entitlement.cloudAi || !capabilities.readiness.access.cloudAiEntitled) {
    return disabled(
      'premium-required',
      'Cloud AI requires Zemara Premium',
  ...
  if (capabilities.readiness.access.globallyDisabled) {
    return disabled(
      'server-unconfigured',
      'Cloud AI disabled',
      'Cloud AI is disabled on the Zemara server.',
      inputModeLabels,
      quotaLabel
    )
  }
  ...
  if (!capabilities.readiness.gateway.configured) {
    return disabled(
      ...
      capabilities.readiness.gateway.configError ?? 'Zemara AI Gateway is not configured.',
      inputModeLabels,
      quotaLabel
    )
  }
  ...
  if (!capabilities.readiness.worker.configured) {
    return disabled(
      ...
      capabilities.readiness.worker.configError ?? 'Zemara AI worker is not configured.',
      inputModeLabels,
      quotaLabel
    )
  }
  ...
  if (!capabilities.readiness.transcription.configured && !hasTranscriptMode) {
    return disabled(
      ...
      capabilities.readiness.transcription.configError ??
        'Zemara cloud transcription is not configured.',
      inputModeLabels,
      quotaLabel
    )
  }
  ...
  if (!capabilities.features.cloudAiEnabled) {
    return disabled(
      'server-unconfigured',
      'Cloud AI not ready',
      'Zemara cloud AI is not ready for this account.',
      inputModeLabels,
      quotaLabel
    )
  }
