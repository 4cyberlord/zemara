...
function accessDetail(state: SystemAccessState, subject: string, os: AppPlatform): string {
  ...
  if (os === 'win32' && (state === 'not-granted' || state === 'first-use')) {
    ...
    // and does NOT list them by name — the tester was hunting for "Zemara" in
    // a list where it can never appear. Point at the two real levers instead.
    return `In ${settings} → Privacy & security → ${windowsPrivacyPageName(subject)}, turn on access and “Let desktop apps access your ${windowsDeviceNoun(subject)}”. Zemara is a desktop app, so it isn’t listed by name.`
  }
  switch (state) {
    case 'granted':
