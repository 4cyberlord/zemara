...
// frame window. Budget it separately, below the 8 s live output proof.
// A cold camera device (first open after a fresh permission grant) adds its
// own startup latency on top of that — AVFoundation's device power-on alone
// commonly takes 1-2s — so a layout switch that brings the camera into frame
// for the first time needs materially more slack than a screen-only switch.
const NATIVE_PREVIEW_SCENE_PROOF_WAIT_TIMEOUT_MS = 8000
const LIVE_LAYOUT_PROOF_WAIT_TIMEOUT_MS = 8000
...
export type StudioContextValue = {
  ...
  // Forces an immediate open attempt for the given camera, independent of
  // whether the current scene's layout actually uses a camera. This is what
  // makes selecting a not-yet-authorized camera trigger the real macOS
  // permission prompt right away, instead of silently doing nothing.
  requestCameraPermission: (cameraId: string) => Promise<PreviewCameraStatus | null>
...
export function StudioProvider({ children }: { children: ReactNode }): ReactElement {
  ...
  const authorizeXLive = useCallback(async () => {
    ...
    try {
      ...
      toast.success('Approve Zemara on x.com to enable X Live.')
    } catch (error) {
      reportError(error)
    }
    ...
    const offLifecycle = window.videorc.onBackendLifecycle?.((event) => {
      if (event.state === 'restarting') {
        ...
        toast.error('Backend crashed repeatedly', {
          id: 'backend-lifecycle',
          description: 'Automatic restarts stopped. Restart Zemara to recover.',
          duration: Infinity
        })
        ...
        if (!alreadyToasted) {
          if (event.status === 'verified') {
            ...
            toast.warning('X is still provisioning playback.', {
              description:
                'Viewers may see a loading spinner for a few minutes. Keep streaming — Zemara keeps checking.'
            })
          } else {
  ...
  const rememberLayoutCommit = useCallback(
    async (
      ...
      // scrolled away) or simply never opened, the backend never starts a
      // compositor render loop for it (see start_synthetic_compositor's only
      // non-recording caller in preview_surface.rs) — frame_scene_revision
      // then never advances, so waitForPreviewLayoutProof's poll is
      // unsatisfiable no matter how long it waits. Skip straight to the
      // committed status in that case; there is nothing rendering to prove.
  ...
  const ensureNativePreviewCamera = useCallback(async () => {
    ...
    // Pre-warm deliberately, regardless of the active preset: the backend now
    // keeps a source alive for minutes of disuse (UNUSED_SOURCE_STOP_GRACE), so
    // starting the camera here even on Screen-only means the first click onto a
    // camera-containing scene later hits the already-live hot path instead of
    // paying a real device-open cost at the moment the user is waiting on it.
  ...
  // Fires the start request the moment a camera is chosen, rather than
  // waiting on captureConfig state to propagate through to
  // ensureNativePreviewCamera's own effect. Selecting a camera that only
  // needs permission would otherwise sit idle for a render cycle before the
  // backend ever reaches the code path that triggers the real system
  // permission dialog — mirroring how picking a microphone already works.
  const requestCameraPermission = useCallback(
    async (cameraId: string): Promise<PreviewCameraStatus | null> => {
      // Set the selection immediately, synchronously, before the request
      // even goes out. The dropdown's value is bound to this state — if we
      // wait for the (async, possibly failing/throwing) backend round trip
      // before touching it, any error below leaves state untouched and the
      // Select silently snaps back to its previous value with zero visible
      // feedback, which reads as "I picked it and it disappeared."
      setCaptureConfig((current) => ({
        ...current,
        sources: { ...current.sources, cameraId }
      }))

      if (!client || wsStatus !== 'connected') {
        reportError(new Error('Not connected to the capture backend yet — try again in a moment.'))
        return null
      }

      try {
        const status = await client.request<PreviewCameraStatus>('preview.camera.start', {
          sources: { ...captureConfig.sources, cameraId },
          layout: captureConfig.layout,
          video: captureConfig.video
        })
        applyPreviewCameraStatus(status)
        try {
          const nextDevices = await client.request<DeviceList>('devices.list')
          setDeviceList(nextDevices)
        } catch (error) {
          console.error('Refreshing device list after camera permission request failed:', error)
        }
        return status
      } catch (error) {
        reportError(error)
  ...
  const ensureNativePreviewScreen = useCallback(async () => {
    ...
    // Pre-warm deliberately, regardless of the active preset — mirrors the same
    // choice in ensureNativePreviewCamera above. The backend already tolerates
    // an unneeded source staying live (UNUSED_SOURCE_STOP_GRACE); starting it
    // here means switching onto a screen-containing scene later is instant
    // instead of paying its device-open cost at click time.
  ...
  useEffect(() => {
    ...
    const exhaustRetries = (envelope: AccountCallbackEnvelope): void => {
      ...
      toast.error(
        'Account sign-in is still unavailable. Zemara kept the callback without acknowledging it.'
      )
    }
    const scheduleRetry = (envelope: AccountCallbackEnvelope): void => {
      ...
      if (attempt === 0) {
        toast.error('Account sign-in is temporarily unavailable. Zemara will retry.')
      }
      const timer = window.setTimeout(() => {
  ...
  useEffect(() => {
    ...
    const exhaustRetries = (envelope: OAuthCallbackEnvelope): void => {
      ...
      exhaustedCallbackIds.add(envelope.id)
      toast.error(
        'OAuth completion is still unavailable. Zemara kept the callback without acknowledging it.'
      )
    }
    const scheduleRetry = (envelope: OAuthCallbackEnvelope): void => {
      ...
      retryAttempts.set(envelope.id, attempt + 1)
      if (attempt === 0) {
        toast.error('OAuth completion is temporarily unavailable. Zemara will retry.')
      }
      const timer = window.setTimeout(() => {
        retryTimers.delete(timer)
        inFlightCallbacks.delete(envelope.id)
  ...
  // ("After ending, stop your encoder"). Zemara used to do the opposite —
