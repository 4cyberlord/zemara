...
export const electronInvokeApiMethods = {
  ...
  'app:quit': 'quitApp',
  'app:set-window-gated': 'setWindowGated',
  ...
  'youtube-bridge:get-embed-url': 'getYoutubeEmbedUrl',
  ...
  'livingText-output:open': 'openLivingTextOutput',
  'livingText-output:close': 'closeLivingTextOutput',
  'livingText-output:get-state': 'getLivingTextOutputState',
  'livingText-output:push': 'pushLivingTextOutput',
  ...
  'system:export-text-file': 'exportTextFile',
...
export interface ElectronIpcEventMap {
  ...
  'livingText-output:state': LivingTextOutputState
  'livingText-output:update': LivingTextOutputPayload
...
export const electronEventChannels = [
  ...
  'livingText-output:state',
  'livingText-output:update',
...
const youtubeVideoIdSchema = runtimeSchema<string>('a YouTube video ID', (value, path) => {
  const input = stringSchema({ minLength: 1, maxLength: 32 }).parse(value, path)
  if (!/^[A-Za-z0-9_-]+$/.test(input)) {
    throw new RuntimeSchemaError(path, 'a YouTube video ID')
  }
  return input
})
...
const livingTextOutputThemeSchema = objectSchema(
  {
    backgroundColor: stringSchema({ minLength: 1, maxLength: 32 }),
    textColor: stringSchema({ minLength: 1, maxLength: 32 }),
    referenceColor: stringSchema({ minLength: 1, maxLength: 32 }),
    fontSize: numberSchema({ min: 8, max: 400 }),
    textAlign: enumSchema(['left', 'center', 'right'])
  },
  { allowUnknown: false }
)
const livingTextOutputVerseSchema = objectSchema(
  {
    text: stringSchema({ minLength: 1, maxLength: 8192 }),
    reference: stringSchema({ minLength: 1, maxLength: 256 })
  },
  { allowUnknown: false }
)
const livingTextOutputPayloadSchema = objectSchema(
  {
    theme: livingTextOutputThemeSchema,
    verse: nullableSchema(livingTextOutputVerseSchema)
  },
  { allowUnknown: false }
)
const livingTextOutputStateSchema = objectSchema({ open: booleanSchema }, { allowUnknown: false })
...
const specificRuntimeInvokeContracts = {
  ...
  'youtube-bridge:get-embed-url': invokeContract(tupleSchema([youtubeVideoIdSchema]), boundedUrl),
  ...
  'livingText-output:push': invokeContract(
    tupleSchema([livingTextOutputPayloadSchema]),
    undefinedSchema
  ),
  ...
  'app:quit': invokeContract(noArgs, undefinedSchema),
  'app:set-window-gated': invokeContract(tupleSchema([booleanSchema]), undefinedSchema)
...
export const boundedPassthroughElectronInvokeChannels = [
  ...
  'livingText-output:open',
  'livingText-output:close',
  'livingText-output:get-state',
...
const specificRuntimeEventSchemas = {
  ...
  'livingText-output:state': livingTextOutputStateSchema,
  'livingText-output:update': livingTextOutputPayloadSchema,
  'oauth:callback-url': oauthCallbackEnvelopeSchema,
  'shortcut:navigate': enumSchema([
    '1',
    '2',
    '3',
    '4',
