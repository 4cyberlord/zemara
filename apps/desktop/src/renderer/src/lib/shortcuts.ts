...
export const SHORTCUTS: readonly ShortcutEntry[] = [
  ...
  { id: 'nav-sermons', keys: ['⌘', 'R'], label: 'Sermons', group: 'Navigation' },
  { id: 'nav-videos', keys: ['⌘', 'I'], label: 'Videos', group: 'Navigation' },
  { id: 'nav-events', keys: ['⌘', 'E'], label: 'Events', group: 'Navigation' },
  { id: 'nav-members', keys: ['⌘', 'B'], label: 'Members', group: 'Navigation' },
  { id: 'nav-alerts', keys: ['⌘', 'L'], label: 'Alerts', group: 'Navigation' },
  { id: 'nav-departments', keys: ['⌘', 'D'], label: 'Departments', group: 'Navigation' },
  { id: 'nav-ai', keys: ['⌘', 'U'], label: 'Publish', group: 'Navigation' },
  { id: 'nav-staffAccess', keys: ['⌘', 'A'], label: 'Staff & Access', group: 'Navigation' },
...
/** Navigation key for a workspace tab id ("nav-<tab>" convention) — a digit,
 *  ',', or one of the single-letter mnemonics (R/I/E/B/L/U). */
export function navShortcutDigit(tab: string): string | undefined {
  ...
  if (!digit || !(/^[1-9]$/.test(digit) || digit === ',' || /^[A-Z]$/.test(digit))) {
    return undefined
  }
  return digit
}
