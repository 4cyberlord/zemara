...
export type RendererRole = 'main' | 'notes' | 'comments' | 'captions' | 'livingTextOutput'
...
export const IPC_INVOKE_ROLES = {
  ...
  'app:quit': MAIN_ONLY,
  'app:set-window-gated': MAIN_ONLY,
  ...
  'system:export-text-file': MAIN_ONLY,
  ...
  'youtube-bridge:get-embed-url': MAIN_ONLY,
  ...
  'livingText-output:open': MAIN_ONLY,
  'livingText-output:close': MAIN_ONLY,
  'livingText-output:get-state': MAIN_ONLY,
  'livingText-output:push': MAIN_ONLY,
...
export const RENDERER_DOCUMENT_CSP = [
  ...
  // https: covers admin-authored thumbnail URLs (same trust tier as media-src
  // below — Sermons/Videos content sourced from the operator's own admin
  // backend, not arbitrary page content).
  "img-src 'self' data: blob: file: videorc-asset: https: http://127.0.0.1:* http://localhost:*",
  "font-src 'self' data:",
  "media-src 'self' data: blob: file: videorc-asset: https:",
  // http://127.0.0.1:* (not https://www.youtube.com) because the YouTube
  // IFrame Player API's origin handshake rejects the packaged app's opaque
  // file:// document origin (undocumented "Error 153"). The Sermons/Videos
  // player embeds a small loopback-only bridge page instead (main process,
  // youtube-bridge:get-embed-url) that runs YT.Player from a real http
  // origin and relays play/pause/seek/volume via postMessage — narrower than
  // the previous youtube.com allowance since this can only ever point at our
  // own local server, not a remote host.
  'frame-src http://127.0.0.1:*',
  // tiles.openfreemap.org serves every resource the Events location map needs
  // (vector tiles, sprite, glyphs) from this one host — confirmed by
  // inspecting the 'liberty' style JSON, so no other map-CDN host is needed.
  "connect-src 'self' videorc-asset: https://www.videorc.com https://tiles.openfreemap.org http://127.0.0.1:* http://localhost:* ws://127.0.0.1:* ws://localhost:*",
