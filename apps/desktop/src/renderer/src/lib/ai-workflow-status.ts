...
export function activeAiWorkflowStatus(session: SessionSummary): AiWorkflowStatus {
  ...
  return hasLocalInput
    ? {
        description: 'Zemara is waiting for the server job to return generated artifacts.',
        title: 'Processing cloud AI',
        tone: 'neutral'
      }
    : {
        description: 'Zemara is preparing the local transcript or audio before any cloud upload.',
