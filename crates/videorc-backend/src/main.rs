...
async fn handle_text_message_with_role(
    ...
    let mut response = match command.method.as_str() {
        ...
        "admin.connection.get" => match prophet_admin_api::get_connection() {
            Ok(connection) => ServerResponse::ok(command.id, connection),
            Err(error) => {
                ServerResponse::error(command.id, "admin-connection-read-failed", error.to_string())
            }
        },
        "admin.connection.set" => {
            match serde_json::from_value::<protocol::AdminConnectionSetParams>(command.params) {
                Ok(params) => {
                    match prophet_admin_api::set_connection(
                        &params.base_url,
                        params.api_key.as_deref(),
                    ) {
                        Ok(()) => match prophet_admin_api::get_connection() {
                            Ok(connection) => ServerResponse::ok(command.id, connection),
                            Err(error) => ServerResponse::error(
                                command.id,
                                "admin-connection-read-failed",
                                error.to_string(),
                            ),
                        },
                        Err(error) => ServerResponse::error(
                            command.id,
                            "admin-connection-save-failed",
                            error.to_string(),
                        ),
                    }
                }
        ...
        "admin.connection.clear" => match prophet_admin_api::clear_connection() {
            Ok(()) => match prophet_admin_api::get_connection() {
                Ok(connection) => ServerResponse::ok(command.id, connection),
                Err(error) => ServerResponse::error(
                    command.id,
            ...
            Err(error) => {
                ServerResponse::error(command.id, "admin-connection-clear-failed", error.to_string())
            }
        },
        "admin.connection.test" => {
            match serde_json::from_value::<protocol::AdminConnectionTestParams>(command.params) {
                Ok(params) => {
                    let result =
                        prophet_admin_api::test_connection(&params.base_url, &params.api_key).await;
                    ServerResponse::ok(command.id, result)
                }
                Err(error) => {
                    ServerResponse::error(command.id, "invalid-params", error.to_string())
        ...
        "admin.register" => {
            match serde_json::from_value::<protocol::AdminRegisterParams>(command.params) {
                Ok(params) => match prophet_admin_api::register_organization(
                    &params.base_url,
                ...
                .await
                {
                    Ok(result) => ServerResponse::ok(command.id, result),
                    Err(error) => {
                        ServerResponse::error(command.id, "admin-register-failed", error.to_string())
                    }
                },
                Err(error) => {
                    ServerResponse::error(command.id, "invalid-params", error.to_string())
        ...
        "livingText.books.list" => ServerResponse::ok(command.id, living_text::list_books()),
        "livingText.availableTranslations.list" => {
            match prophet_admin_api::ProphetAdminApiClient::connect() {
                Ok(admin) => match admin.list_bible_translations().await {
                    Ok(translations) => ServerResponse::ok(command.id, translations),
                    Err(error) => ServerResponse::error(
                        command.id,
                        "living-text-available-translations-failed",
                        error.to_string(),
                    ),
                },
                Err(error) => ServerResponse::error(
                    command.id,
                    "admin-connection-not-configured",
                    error.to_string(),
                ),
            }
        }
        "livingText.translations.list" => match living_text::LivingTextStore::open() {
            Ok(store) => match store.list_translations() {
                Ok(translations) => ServerResponse::ok(command.id, translations),
                Err(error) => ServerResponse::error(
                    command.id,
                    "living-text-translations-list-failed",
                    error.to_string(),
                ),
            },
            Err(error) => {
                ServerResponse::error(command.id, "living-text-store-unavailable", error.to_string())
            }
        },
        "livingText.sync" => {
            match serde_json::from_value::<protocol::LivingTextSyncParams>(command.params) {
                Ok(params) => match prophet_admin_api::ProphetAdminApiClient::connect() {
                    Ok(admin) => match living_text::LivingTextStore::open() {
                        Ok(mut store) => match store.sync_translation(&admin, &params.translation_code).await
                        {
                            Ok(summary) => ServerResponse::ok(command.id, summary),
                            Err(error) => ServerResponse::error(
                                command.id,
                                "living-text-sync-failed",
                                error.to_string(),
                            ),
                        },
                        Err(error) => ServerResponse::error(
                            command.id,
                            "living-text-store-unavailable",
                            error.to_string(),
                        ),
                    },
                    Err(error) => ServerResponse::error(
        ...
        "livingText.chapter.get" => {
            match serde_json::from_value::<protocol::LivingTextChapterParams>(command.params) {
                Ok(params) => match living_text::LivingTextStore::open() {
                    Ok(store) => match store.get_chapter(
                        &params.translation_code,
                        &params.book,
                        params.chapter,
                    ) {
                        Ok(verses) => ServerResponse::ok(command.id, verses),
                        Err(error) => ServerResponse::error(
                            command.id,
                            "living-text-chapter-failed",
                            error.to_string(),
                        ),
                    },
                    Err(error) => ServerResponse::error(
        ...
        "livingText.detect" => {
            match serde_json::from_value::<protocol::LivingTextDetectParams>(command.params) {
                Ok(params) => match living_text::LivingTextStore::open() {
                    Ok(store) => match store.detect_and_resolve(&params.translation_code, &params.text) {
                        Ok(citations) => ServerResponse::ok(command.id, citations),
                        Err(error) => ServerResponse::error(
                            command.id,
                            "living-text-detect-failed",
                            error.to_string(),
                        ),
                    },
                    Err(error) => ServerResponse::error(
        ...
        "admin.sermons.list" => match prophet_admin_api::ProphetAdminApiClient::connect() {
            Ok(client) => match client.list_sermons().await {
                Ok(sermons) => ServerResponse::ok(command.id, sermons),
                Err(error) => {
                    ServerResponse::error(command.id, "admin-sermons-list-failed", error.to_string())
                }
            },
            Err(error) => ServerResponse::error(
                command.id,
        ...
        "admin.sermons.create" => {
            match serde_json::from_value::<prophet_admin_api::CreateSermonRequest>(command.params) {
                Ok(request) => match prophet_admin_api::ProphetAdminApiClient::connect() {
                    Ok(client) => match client.create_sermon(&request).await {
                        Ok(sermon) => ServerResponse::ok(command.id, sermon),
                        Err(error) => ServerResponse::error(
                            command.id,
                            "admin-sermon-create-failed",
                            error.to_string(),
                        ),
                    },
                    Err(error) => ServerResponse::error(
        ...
        "admin.sermons.update" => {
            ...
            if id.is_empty() {
                ServerResponse::error(
                    command.id,
                    "invalid-params",
                    "admin.sermons.update requires an id.",
                )
            } else {
                match serde_json::from_value::<prophet_admin_api::UpdateSermonRequest>(
                    command.params,
                ) {
                    Ok(request) => match prophet_admin_api::ProphetAdminApiClient::connect() {
                        Ok(client) => match client.update_sermon(&id, &request).await {
                            Ok(sermon) => ServerResponse::ok(command.id, sermon),
                            Err(error) => ServerResponse::error(
                                command.id,
                                "admin-sermon-update-failed",
                                error.to_string(),
                            ),
                        },
                        Err(error) => ServerResponse::error(
        ...
        "admin.sermons.delete" => {
            ...
            if id.is_empty() {
                ServerResponse::error(
                    command.id,
                    "invalid-params",
                    "admin.sermons.delete requires an id.",
                )
            } else {
                match prophet_admin_api::ProphetAdminApiClient::connect() {
                    Ok(client) => match client.delete_sermon(&id).await {
                        Ok(()) => {
                            ServerResponse::ok(command.id, serde_json::json!({ "deleted": true }))
                        }
                        Err(error) => ServerResponse::error(
                            command.id,
                            "admin-sermon-delete-failed",
                            error.to_string(),
                        ),
                    },
                    Err(error) => ServerResponse::error(
        ...
        "admin.sermons.series.list" => match prophet_admin_api::ProphetAdminApiClient::connect() {
            Ok(client) => match client.list_series().await {
                Ok(series) => ServerResponse::ok(command.id, series),
                Err(error) => {
                    ServerResponse::error(command.id, "admin-series-list-failed", error.to_string())
                }
            },
            Err(error) => ServerResponse::error(
                command.id,
        ...
        "admin.sermons.series.create" => {
            match serde_json::from_value::<prophet_admin_api::CreateSeriesRequest>(command.params) {
                Ok(request) => match prophet_admin_api::ProphetAdminApiClient::connect() {
                    Ok(client) => match client.create_series(&request).await {
                        Ok(series) => ServerResponse::ok(command.id, series),
                        Err(error) => ServerResponse::error(
                            command.id,
                            "admin-series-create-failed",
                            error.to_string(),
                        ),
                    },
                    Err(error) => ServerResponse::error(
        ...
        "admin.videos.list" => match prophet_admin_api::ProphetAdminApiClient::connect() {
            Ok(client) => match client.list_videos().await {
                Ok(videos) => ServerResponse::ok(command.id, videos),
                Err(error) => {
                    ServerResponse::error(command.id, "admin-videos-list-failed", error.to_string())
                }
            },
            Err(error) => ServerResponse::error(
                command.id,
        ...
        "admin.videos.create" => {
            match serde_json::from_value::<prophet_admin_api::CreateVideoRequest>(command.params) {
                Ok(request) => match prophet_admin_api::ProphetAdminApiClient::connect() {
                    Ok(client) => match client.create_video(&request).await {
                        Ok(video) => ServerResponse::ok(command.id, video),
                        Err(error) => ServerResponse::error(
                            command.id,
                            "admin-video-create-failed",
                            error.to_string(),
                        ),
                    },
                    Err(error) => ServerResponse::error(
        ...
        "admin.videos.update" => {
            ...
            if id.is_empty() {
                ServerResponse::error(
                    command.id,
                    "invalid-params",
                    "admin.videos.update requires an id.",
                )
            } else {
                match serde_json::from_value::<prophet_admin_api::UpdateVideoRequest>(command.params)
                {
                    Ok(request) => match prophet_admin_api::ProphetAdminApiClient::connect() {
                        Ok(client) => match client.update_video(&id, &request).await {
                            Ok(video) => ServerResponse::ok(command.id, video),
                            Err(error) => ServerResponse::error(
                                command.id,
                                "admin-video-update-failed",
                                error.to_string(),
                            ),
                        },
                        Err(error) => ServerResponse::error(
        ...
        "admin.videos.delete" => {
            ...
            if id.is_empty() {
                ServerResponse::error(
                    command.id,
                    "invalid-params",
                    "admin.videos.delete requires an id.",
                )
            } else {
                match prophet_admin_api::ProphetAdminApiClient::connect() {
                    Ok(client) => match client.delete_video(&id).await {
                        Ok(()) => {
                            ServerResponse::ok(command.id, serde_json::json!({ "deleted": true }))
                        }
                        Err(error) => ServerResponse::error(
                            command.id,
                            "admin-video-delete-failed",
                            error.to_string(),
                        ),
                    },
                    Err(error) => ServerResponse::error(
        ...
        "admin.videos.toggle_featured" => {
            ...
            if id.is_empty() {
                ServerResponse::error(
                    command.id,
                    "invalid-params",
                    "admin.videos.toggle_featured requires an id.",
                )
            } else {
                match prophet_admin_api::ProphetAdminApiClient::connect() {
                    Ok(client) => match client.toggle_video_featured(&id).await {
                        Ok(is_featured) => {
                            ServerResponse::ok(command.id, serde_json::json!({ "isFeatured": is_featured }))
                        }
                        Err(error) => ServerResponse::error(
                            command.id,
                            "admin-video-toggle-featured-failed",
                            error.to_string(),
                        ),
                    },
                    Err(error) => ServerResponse::error(
        ...
        "admin.videos.categories.list" => match prophet_admin_api::ProphetAdminApiClient::connect()
        {
            Ok(client) => match client.list_video_categories().await {
                Ok(categories) => ServerResponse::ok(command.id, categories),
                Err(error) => ServerResponse::error(
                    command.id,
                    "admin-video-categories-list-failed",
                    error.to_string(),
                ),
            },
            Err(error) => ServerResponse::error(
        ...
        "admin.videos.categories.create" => {
            match serde_json::from_value::<prophet_admin_api::CreateVideoCategoryRequest>(
                command.params,
            ) {
                Ok(request) => match prophet_admin_api::ProphetAdminApiClient::connect() {
                    Ok(client) => match client.create_video_category(&request).await {
                        Ok(category) => ServerResponse::ok(command.id, category),
                        Err(error) => ServerResponse::error(
                            command.id,
                            "admin-video-category-create-failed",
                            error.to_string(),
                        ),
                    },
                    Err(error) => ServerResponse::error(
        ...
        "admin.videos.youtube_search" => {
            let query =
                command.params.get("query").and_then(serde_json::Value::as_str).unwrap_or_default();
            if query.trim().is_empty() {
                ServerResponse::error(
                    command.id,
                    "invalid-params",
                    "admin.videos.youtube_search requires a query.",
                )
            } else {
                match prophet_admin_api::ProphetAdminApiClient::connect() {
                    Ok(client) => match client.search_youtube(query).await {
                        Ok(results) => ServerResponse::ok(command.id, results),
                        Err(error) => ServerResponse::error(
                            command.id,
                            "admin-youtube-search-failed",
                            error.to_string(),
                        ),
                    },
                    Err(error) => ServerResponse::error(
        ...
        "admin.videos.youtube_import" => {
            match serde_json::from_value::<prophet_admin_api::YoutubeImportRequest>(command.params) {
                Ok(request) => match prophet_admin_api::ProphetAdminApiClient::connect() {
                    Ok(client) => match client.import_from_youtube(&request).await {
                        Ok(videos) => ServerResponse::ok(command.id, videos),
                        Err(error) => ServerResponse::error(
                            command.id,
                            "admin-youtube-import-failed",
                            error.to_string(),
                        ),
                    },
                    Err(error) => ServerResponse::error(
            ...
            match prophet_admin_api::ProphetAdminApiClient::connect() {
                Ok(client) => {
                    match client
                        ...
                        .await
                    {
                        Ok(members) => ServerResponse::ok(command.id, members),
                        Err(error) => ServerResponse::error(
                            command.id,
                            "admin-members-list-failed",
                            error.to_string(),
                        ),
                    }
                }
        ...
        "admin.members.create" => {
            match serde_json::from_value::<prophet_admin_api::CreateMemberRequest>(command.params) {
                Ok(request) => match prophet_admin_api::ProphetAdminApiClient::connect() {
                    Ok(client) => match client.create_member(&request).await {
                        Ok(member) => ServerResponse::ok(command.id, member),
                        Err(error) => ServerResponse::error(
                            command.id,
                            "admin-member-create-failed",
                            error.to_string(),
                        ),
                    },
                    Err(error) => ServerResponse::error(
        ...
        "admin.members.update" => {
            ...
            if id.is_empty() {
                ServerResponse::error(
                    command.id,
                    "invalid-params",
                    "admin.members.update requires an id.",
                )
            } else {
                match serde_json::from_value::<prophet_admin_api::UpdateMemberRequest>(command.params)
                {
                    Ok(request) => match prophet_admin_api::ProphetAdminApiClient::connect() {
                        Ok(client) => match client.update_member(&id, &request).await {
                            Ok(member) => ServerResponse::ok(command.id, member),
                            Err(error) => ServerResponse::error(
                                command.id,
                                "admin-member-update-failed",
                                error.to_string(),
                            ),
                        },
                        Err(error) => ServerResponse::error(
        ...
        "admin.members.delete" => {
            ...
            if id.is_empty() {
                ServerResponse::error(
                    command.id,
                    "invalid-params",
                    "admin.members.delete requires an id.",
                )
            } else {
                match prophet_admin_api::ProphetAdminApiClient::connect() {
                    Ok(client) => match client.delete_member(&id).await {
                        Ok(()) => {
                            ServerResponse::ok(command.id, serde_json::json!({ "deleted": true }))
                        }
                        Err(error) => ServerResponse::error(
                            command.id,
                            "admin-member-delete-failed",
                            error.to_string(),
                        ),
                    },
                    Err(error) => ServerResponse::error(
        ...
        "admin.notifications.list" => match prophet_admin_api::ProphetAdminApiClient::connect() {
            Ok(client) => match client.list_campaigns().await {
                Ok(campaigns) => ServerResponse::ok(command.id, campaigns),
                Err(error) => ServerResponse::error(
                    command.id,
        ...
        "admin.notifications.create" => {
            match serde_json::from_value::<prophet_admin_api::CreateCampaignRequest>(command.params)
            {
                Ok(request) => match prophet_admin_api::ProphetAdminApiClient::connect() {
                    Ok(client) => match client.create_campaign(&request).await {
                        Ok(campaign) => ServerResponse::ok(command.id, campaign),
                        Err(error) => ServerResponse::error(
                            command.id,
        ...
        "admin.notifications.update" => {
            ...
            if id.is_empty() {
                ServerResponse::error(
                    command.id,
                    "invalid-params",
                    "admin.notifications.update requires an id.",
                )
            } else {
                match serde_json::from_value::<prophet_admin_api::UpdateCampaignRequest>(
                    command.params,
                ) {
                    Ok(request) => match prophet_admin_api::ProphetAdminApiClient::connect() {
                        Ok(client) => match client.update_campaign(&id, &request).await {
                            Ok(campaign) => ServerResponse::ok(command.id, campaign),
                            Err(error) => ServerResponse::error(
                                command.id,
                                "admin-notification-update-failed",
                                error.to_string(),
                            ),
                        },
                        Err(error) => ServerResponse::error(
        ...
        "admin.notifications.delete" => {
            ...
            if id.is_empty() {
                ServerResponse::error(
                    command.id,
                    "invalid-params",
                    "admin.notifications.delete requires an id.",
                )
            } else {
                match prophet_admin_api::ProphetAdminApiClient::connect() {
                    Ok(client) => match client.delete_campaign(&id).await {
                        Ok(()) => {
                            ServerResponse::ok(command.id, serde_json::json!({ "deleted": true }))
                        }
                        Err(error) => ServerResponse::error(
                            command.id,
                            "admin-notification-delete-failed",
                            error.to_string(),
                        ),
                    },
                    Err(error) => ServerResponse::error(
        ...
        "admin.notifications.send" => {
            ...
            if id.is_empty() {
                ServerResponse::error(
                    command.id,
                    "invalid-params",
                    "admin.notifications.send requires an id.",
                )
            } else {
                match prophet_admin_api::ProphetAdminApiClient::connect() {
                    Ok(client) => match client.send_campaign_now(&id).await {
                        Ok(campaign) => ServerResponse::ok(command.id, campaign),
                        Err(error) => ServerResponse::error(
                            command.id,
        ...
        "admin.events.list" => match prophet_admin_api::ProphetAdminApiClient::connect() {
            Ok(client) => match client.list_events().await {
                Ok(events) => ServerResponse::ok(command.id, events),
                Err(error) => {
                    ServerResponse::error(command.id, "admin-events-list-failed", error.to_string())
                }
            },
            Err(error) => ServerResponse::error(
                command.id,
        ...
        "admin.events.create" => {
            match serde_json::from_value::<prophet_admin_api::CreateEventRequest>(command.params) {
                Ok(request) => match prophet_admin_api::ProphetAdminApiClient::connect() {
                    Ok(client) => match client.create_event(&request).await {
        ...
        "admin.events.update" => {
            ...
            if id.is_empty() {
                ServerResponse::error(
                    command.id,
                    "invalid-params",
                    "admin.events.update requires an id.",
                )
            } else {
                match serde_json::from_value::<prophet_admin_api::UpdateEventRequest>(command.params)
                {
                    Ok(request) => match prophet_admin_api::ProphetAdminApiClient::connect() {
                        Ok(client) => match client.update_event(&id, &request).await {
                            Ok(event) => ServerResponse::ok(command.id, event),
                            Err(error) => ServerResponse::error(
                                command.id,
        ...
        "admin.events.delete" => {
            ...
            if id.is_empty() {
                ServerResponse::error(
                    command.id,
                    "invalid-params",
                    "admin.events.delete requires an id.",
                )
            } else {
                match prophet_admin_api::ProphetAdminApiClient::connect() {
                    Ok(client) => match client.delete_event(&id).await {
                        Ok(()) => {
                            ServerResponse::ok(command.id, serde_json::json!({ "deleted": true }))
                        }
                        Err(error) => ServerResponse::error(
        ...
        "admin.attendance.open" => {
            ...
            if event_id.is_empty() {
                ServerResponse::error(command.id, "invalid-params", "admin.attendance.open requires an eventId.")
            } else {
                match prophet_admin_api::ProphetAdminApiClient::connect() {
                    Ok(client) => match client.open_attendance(&event_id).await {
                        Ok(session) => ServerResponse::ok(command.id, session),
                        Err(error) => {
                            ServerResponse::error(command.id, "attendance-open-failed", error.to_string())
                        }
                    },
                    Err(error) => {
                        ServerResponse::error(command.id, "admin-connection-not-configured", error.to_string())
                    }
                }
            }
        }
        "admin.attendance.close" => {
            let event_id = command
                ...
                .to_string();
            if event_id.is_empty() {
                ServerResponse::error(command.id, "invalid-params", "admin.attendance.close requires an eventId.")
            } else {
                match prophet_admin_api::ProphetAdminApiClient::connect() {
                    Ok(client) => match client.close_attendance(&event_id).await {
                        Ok(session) => ServerResponse::ok(command.id, session),
                        Err(error) => {
                            ServerResponse::error(command.id, "attendance-close-failed", error.to_string())
                        }
                    },
                    Err(error) => {
                        ServerResponse::error(command.id, "admin-connection-not-configured", error.to_string())
        ...
        "admin.attendance.qr" => {
            let event_id = command
                ...
                .to_string();
            if event_id.is_empty() {
                ServerResponse::error(command.id, "invalid-params", "admin.attendance.qr requires an eventId.")
            } else {
                match prophet_admin_api::ProphetAdminApiClient::connect() {
                    Ok(client) => match client.get_attendance_qr(&event_id).await {
                        Ok(qr) => ServerResponse::ok(command.id, qr),
                        Err(error) => {
                            ServerResponse::error(command.id, "attendance-qr-failed", error.to_string())
                        }
                    },
                    Err(error) => {
                        ServerResponse::error(command.id, "admin-connection-not-configured", error.to_string())
        ...
        "admin.attendance.list" => {
            let event_id = command
                ...
                .to_string();
            if event_id.is_empty() {
                ServerResponse::error(command.id, "invalid-params", "admin.attendance.list requires an eventId.")
            } else {
                match prophet_admin_api::ProphetAdminApiClient::connect() {
                    Ok(client) => match client.list_attendance(&event_id).await {
                        Ok(list) => ServerResponse::ok(command.id, list),
                        Err(error) => {
                            ServerResponse::error(command.id, "attendance-list-failed", error.to_string())
                        }
                    },
                    Err(error) => {
                        ServerResponse::error(command.id, "admin-connection-not-configured", error.to_string())
        ...
        "admin.attendance.manual" => {
            let event_id = command
                ...
                .to_string();
            if event_id.is_empty() {
                ServerResponse::error(command.id, "invalid-params", "admin.attendance.manual requires an eventId.")
            } else {
                match serde_json::from_value::<prophet_admin_api::ManualCheckInRequest>(command.params.clone()) {
                    Ok(request) => match prophet_admin_api::ProphetAdminApiClient::connect() {
                        Ok(client) => match client.manual_check_in(&event_id, &request).await {
                            Ok(record) => ServerResponse::ok(command.id, record),
                            Err(error) => {
                                ServerResponse::error(command.id, "attendance-manual-failed", error.to_string())
                            }
                        },
                        Err(error) => ServerResponse::error(
                            command.id,
        ...
        "admin.attendance.delete" => {
            ...
            if event_id.is_empty() || record_id.is_empty() {
                ServerResponse::error(
                    command.id,
                    "invalid-params",
                    "admin.attendance.delete requires an eventId and recordId.",
                )
            } else {
                match prophet_admin_api::ProphetAdminApiClient::connect() {
                    Ok(client) => match client.delete_attendance_record(&event_id, &record_id).await {
                        Ok(()) => {
                            ServerResponse::ok(command.id, serde_json::json!({ "deleted": true }))
                        }
                        Err(error) => {
                            ServerResponse::error(command.id, "attendance-delete-failed", error.to_string())
                        }
                    },
                    Err(error) => {
                        ServerResponse::error(command.id, "admin-connection-not-configured", error.to_string())
        ...
        "admin.units.list" => match prophet_admin_api::ProphetAdminApiClient::connect() {
            Ok(client) => match client.list_units().await {
                Ok(units) => ServerResponse::ok(command.id, units),
                Err(error) => {
                    ServerResponse::error(command.id, "admin-units-list-failed", error.to_string())
                }
            },
            Err(error) => ServerResponse::error(
                command.id,
        ...
        "admin.units.get" => {
            let id = command
                ...
                .to_string();
            if id.is_empty() {
                ServerResponse::error(command.id, "invalid-params", "admin.units.get requires an id.")
            } else {
                match prophet_admin_api::ProphetAdminApiClient::connect() {
                    Ok(client) => match client.get_unit(&id).await {
                        Ok(unit) => ServerResponse::ok(command.id, unit),
                        Err(error) => {
                            ServerResponse::error(command.id, "admin-unit-get-failed", error.to_string())
                        }
                    },
                    Err(error) => {
                        ServerResponse::error(command.id, "admin-connection-not-configured", error.to_string())
        ...
        "admin.units.create" => {
            match serde_json::from_value::<prophet_admin_api::CreateUnitRequest>(command.params) {
                Ok(request) => match prophet_admin_api::ProphetAdminApiClient::connect() {
                    Ok(client) => match client.create_unit(&request).await {
                        Ok(unit) => ServerResponse::ok(command.id, unit),
                        Err(error) => ServerResponse::error(
                            command.id,
                            "admin-unit-create-failed",
                            error.to_string(),
                        ),
                    },
                    Err(error) => ServerResponse::error(
        ...
        "admin.units.update" => {
            ...
            if id.is_empty() {
                ServerResponse::error(
                    command.id,
                    "invalid-params",
                    "admin.units.update requires an id.",
                )
            } else {
                match serde_json::from_value::<prophet_admin_api::UpdateUnitRequest>(command.params) {
                    Ok(request) => match prophet_admin_api::ProphetAdminApiClient::connect() {
                        Ok(client) => match client.update_unit(&id, &request).await {
                            Ok(unit) => ServerResponse::ok(command.id, unit),
                            Err(error) => ServerResponse::error(
                                command.id,
                                "admin-unit-update-failed",
                                error.to_string(),
                            ),
                        },
                        Err(error) => ServerResponse::error(
        ...
        "admin.units.delete" => {
            ...
            if id.is_empty() {
                ServerResponse::error(
                    command.id,
                    "invalid-params",
                    "admin.units.delete requires an id.",
                )
            } else {
                match prophet_admin_api::ProphetAdminApiClient::connect() {
                    Ok(client) => match client.delete_unit(&id).await {
                        Ok(()) => {
                            ServerResponse::ok(command.id, serde_json::json!({ "deleted": true }))
                        }
                        Err(error) => ServerResponse::error(
                            command.id,
                            "admin-unit-delete-failed",
                            error.to_string(),
                        ),
                    },
                    Err(error) => ServerResponse::error(
        ...
        "admin.units.members.add" => {
            let unit_id = command
                ...
                .to_string();
            if unit_id.is_empty() {
                ServerResponse::error(command.id, "invalid-params", "admin.units.members.add requires a unitId.")
            } else {
                match serde_json::from_value::<prophet_admin_api::AddUnitMemberRequest>(command.params.clone()) {
                    Ok(request) => match prophet_admin_api::ProphetAdminApiClient::connect() {
                        Ok(client) => match client.add_unit_member(&unit_id, &request).await {
                            Ok(membership) => ServerResponse::ok(command.id, membership),
                            Err(error) => {
                                ServerResponse::error(command.id, "unit-member-add-failed", error.to_string())
                            }
                        },
                        Err(error) => ServerResponse::error(
                            command.id,
        ...
        "admin.units.members.remove" => {
            let unit_id = command
                ...
                .unwrap_or_default()
                .to_string();
            let membership_id = command
                .params
                .get("membershipId")
                .and_then(serde_json::Value::as_str)
                .unwrap_or_default()
                .to_string();
            if unit_id.is_empty() || membership_id.is_empty() {
                ServerResponse::error(
                    command.id,
                    "invalid-params",
                    "admin.units.members.remove requires a unitId and membershipId.",
                )
            } else {
                match prophet_admin_api::ProphetAdminApiClient::connect() {
                    Ok(client) => match client.remove_unit_member(&unit_id, &membership_id).await {
                        Ok(()) => {
                            ServerResponse::ok(command.id, serde_json::json!({ "deleted": true }))
                        }
                        Err(error) => {
                            ServerResponse::error(command.id, "unit-member-remove-failed", error.to_string())
                        }
                    },
                    Err(error) => {
                        ServerResponse::error(command.id, "admin-connection-not-configured", error.to_string())
        ...
        "admin.units.positions.create" => {
            let unit_id = command
                ...
                .to_string();
            if unit_id.is_empty() {
                ServerResponse::error(command.id, "invalid-params", "admin.units.positions.create requires a unitId.")
            } else {
                match serde_json::from_value::<prophet_admin_api::CreateUnitPositionRequest>(command.params.clone()) {
                    Ok(request) => match prophet_admin_api::ProphetAdminApiClient::connect() {
                        Ok(client) => match client.create_unit_position(&unit_id, &request).await {
                            Ok(position) => ServerResponse::ok(command.id, position),
                            Err(error) => {
                                ServerResponse::error(command.id, "unit-position-create-failed", error.to_string())
                            }
                        },
                        Err(error) => ServerResponse::error(
                            command.id,
        ...
        "admin.units.positions.delete" => {
            let unit_id = command
                ...
                .unwrap_or_default()
                .to_string();
            let position_id = command
                .params
                .get("positionId")
                .and_then(serde_json::Value::as_str)
                .unwrap_or_default()
                .to_string();
            if unit_id.is_empty() || position_id.is_empty() {
                ServerResponse::error(
                    command.id,
                    "invalid-params",
                    "admin.units.positions.delete requires a unitId and positionId.",
                )
            } else {
                match prophet_admin_api::ProphetAdminApiClient::connect() {
                    Ok(client) => match client.delete_unit_position(&unit_id, &position_id).await {
                        Ok(()) => {
                            ServerResponse::ok(command.id, serde_json::json!({ "deleted": true }))
                        }
                        Err(error) => {
                            ServerResponse::error(command.id, "unit-position-delete-failed", error.to_string())
                        }
                    },
                    Err(error) => {
                        ServerResponse::error(command.id, "admin-connection-not-configured", error.to_string())
        ...
        "admin.units.positions.assign" => {
            ...
            if unit_id.is_empty() || position_id.is_empty() {
                ServerResponse::error(
                    command.id,
                    "invalid-params",
                    "admin.units.positions.assign requires a unitId and positionId.",
                )
            } else {
                match serde_json::from_value::<prophet_admin_api::AssignUnitPositionRequest>(command.params.clone()) {
                    Ok(request) => match prophet_admin_api::ProphetAdminApiClient::connect() {
                        Ok(client) => match client.assign_unit_position(&unit_id, &position_id, &request).await {
                            Ok(assignment) => ServerResponse::ok(command.id, assignment),
                            Err(error) => {
                                ServerResponse::error(command.id, "unit-position-assign-failed", error.to_string())
                            }
                        },
                        Err(error) => ServerResponse::error(
                            command.id,
        ...
        "admin.units.positions.assignments.end" => {
            ...
            let assignment_id = command
                ...
                .unwrap_or_default()
                .to_string();
            if unit_id.is_empty() || position_id.is_empty() || assignment_id.is_empty() {
                ServerResponse::error(
                    command.id,
                    "invalid-params",
                    "admin.units.positions.assignments.end requires a unitId, positionId, and assignmentId.",
                )
            } else {
                match prophet_admin_api::ProphetAdminApiClient::connect() {
                    Ok(client) => match client.end_unit_position_assignment(&unit_id, &position_id, &assignment_id).await {
                        Ok(assignment) => ServerResponse::ok(command.id, assignment),
                        Err(error) => {
                            ServerResponse::error(command.id, "unit-position-assignment-end-failed", error.to_string())
                        }
                    },
                    Err(error) => {
                        ServerResponse::error(command.id, "admin-connection-not-configured", error.to_string())
        ...
        "admin.locations.list" => match prophet_admin_api::ProphetAdminApiClient::connect() {
            Ok(client) => match client.list_locations().await {
                Ok(locations) => ServerResponse::ok(command.id, locations),
                Err(error) => {
                    ServerResponse::error(command.id, "admin-locations-list-failed", error.to_string())
                }
            },
            Err(error) => ServerResponse::error(
                command.id,
        ...
        "admin.locations.create" => {
            match serde_json::from_value::<prophet_admin_api::CreateLocationRequest>(command.params) {
                Ok(request) => match prophet_admin_api::ProphetAdminApiClient::connect() {
                    Ok(client) => match client.create_location(&request).await {
                        Ok(location) => ServerResponse::ok(command.id, location),
                        Err(error) => ServerResponse::error(
                            command.id,
                            "admin-location-create-failed",
                            error.to_string(),
                        ),
                    },
                    Err(error) => ServerResponse::error(
        ...
        "admin.locations.update" => {
            ...
            if id.is_empty() {
                ServerResponse::error(
                    command.id,
                    "invalid-params",
                    "admin.locations.update requires an id.",
                )
            } else {
                match serde_json::from_value::<prophet_admin_api::UpdateLocationRequest>(command.params) {
                    Ok(request) => match prophet_admin_api::ProphetAdminApiClient::connect() {
                        Ok(client) => match client.update_location(&id, &request).await {
                            Ok(location) => ServerResponse::ok(command.id, location),
                            Err(error) => ServerResponse::error(
                                command.id,
                                "admin-location-update-failed",
                                error.to_string(),
                            ),
                        },
                        Err(error) => ServerResponse::error(
        ...
        "admin.locations.delete" => {
            ...
            if id.is_empty() {
                ServerResponse::error(
                    command.id,
                    "invalid-params",
                    "admin.locations.delete requires an id.",
                )
            } else {
                match prophet_admin_api::ProphetAdminApiClient::connect() {
                    Ok(client) => match client.delete_location(&id).await {
                        Ok(()) => {
                            ServerResponse::ok(command.id, serde_json::json!({ "deleted": true }))
                        }
                        Err(error) => ServerResponse::error(
                            command.id,
                            "admin-location-delete-failed",
                            error.to_string(),
                        ),
                    },
                    Err(error) => ServerResponse::error(
        ...
        "admin.team.list" => match prophet_admin_api::ProphetAdminApiClient::connect() {
            Ok(client) => match client.list_team_users().await {
                Ok(users) => ServerResponse::ok(command.id, users),
                Err(error) => {
                    ServerResponse::error(command.id, "admin-team-list-failed", error.to_string())
                }
            },
            Err(error) => ServerResponse::error(
                command.id,
        ...
        "admin.team.create" => {
            match serde_json::from_value::<prophet_admin_api::CreateTeamUserRequest>(command.params) {
                Ok(request) => match prophet_admin_api::ProphetAdminApiClient::connect() {
                    Ok(client) => match client.create_team_user(&request).await {
                        Ok(user) => ServerResponse::ok(command.id, user),
                        Err(error) => ServerResponse::error(
                            command.id,
                            "admin-team-create-failed",
                            error.to_string(),
                        ),
                    },
                    Err(error) => ServerResponse::error(
        ...
        "admin.team.update" => {
            ...
            if id.is_empty() {
                ServerResponse::error(
                    command.id,
                    "invalid-params",
                    "admin.team.update requires an id.",
                )
            } else {
                match serde_json::from_value::<prophet_admin_api::UpdateTeamUserRequest>(command.params) {
                    Ok(request) => match prophet_admin_api::ProphetAdminApiClient::connect() {
                        Ok(client) => match client.update_team_user(&id, &request).await {
                            Ok(user) => ServerResponse::ok(command.id, user),
                            Err(error) => ServerResponse::error(
                                command.id,
                                "admin-team-update-failed",
                                error.to_string(),
                            ),
                        },
                        Err(error) => ServerResponse::error(
        ...
        "admin.team.delete" => {
            ...
            if id.is_empty() {
                ServerResponse::error(
                    command.id,
                    "invalid-params",
                    "admin.team.delete requires an id.",
                )
            } else {
                match prophet_admin_api::ProphetAdminApiClient::connect() {
                    Ok(client) => match client.delete_team_user(&id).await {
                        Ok(()) => {
                            ServerResponse::ok(command.id, serde_json::json!({ "deleted": true }))
                        }
                        Err(error) => ServerResponse::error(
                            command.id,
                            "admin-team-delete-failed",
                            error.to_string(),
                        ),
                    },
                    Err(error) => ServerResponse::error(
        ...
        "admin.uploads.image" => {
            ...
            match decoded_bytes {
                Ok(bytes) => match prophet_admin_api::ProphetAdminApiClient::connect() {
                    Ok(client) => match client.upload_image(&folder, &file_name, &mime_type, bytes).await {
                        Ok(uploaded) => ServerResponse::ok(command.id, uploaded),
                        Err(error) => ServerResponse::error(
                            command.id,
        ...
        "geocoding.search" => {
            let query = command
                ...
                .unwrap_or_default()
                .to_string();
            match prophet_admin_api::geocode_search(&query).await {
                Ok(results) => ServerResponse::ok(command.id, results),
                Err(error) => {
                    ServerResponse::error(command.id, "geocoding-search-failed", error.to_string())
                }
            }
        }
        "geocoding.reverse" => {
            let lat = command.params.get("lat").and_then(serde_json::Value::as_f64).unwrap_or_default();
            let lng = command.params.get("lng").and_then(serde_json::Value::as_f64).unwrap_or_default();
            match prophet_admin_api::geocode_reverse(lat, lng).await {
                Ok(result) => ServerResponse::ok(command.id, result),
                Err(error) => {
                    ServerResponse::error(command.id, "geocoding-reverse-failed", error.to_string())
                }
            }
        }
