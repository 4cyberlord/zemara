...
function attachUpdaterListeners(): void {
  ...
  autoUpdater.on('update-downloaded', (info: UpdateInfo) => {
    ...
    if (Notification.isSupported()) {
      new Notification({
        title: `Zemara ${info.version} is ready`,
        body: 'Restart Zemara to finish updating, or it will apply the next time you quit.',
        silent: true
      }).show()
    }
  })
  ...
  secureIpcHandle('updates:install', async () => {
    ...
    try {
      const admission = await installUpdateWithInterruptionLease(
        ...
        () => acquireInterruption('Installing a downloaded Zemara update.', 'update-install'),
