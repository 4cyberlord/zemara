    ...
    <div className="flex min-h-0 flex-1 flex-col gap-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <AppSelect
          ...
          options={LIBRARY_FILTERS.map((option) => ({ value: option.value, label: option.label }))}
        />
        <Button
          size="sm"
          title={
      ...
      {sessions.length === 0 ? (
        <Empty className="rounded-panel border py-16">
          <EmptyMedia variant="default">
            <IconStack>
              <FileVideo weight="duotone" className="size-4" />
            </IconStack>
          </EmptyMedia>
  ...
  const runNoiseCleanupAction = async (action: NoiseCleanupAction): Promise<void> => {
    ...
    try {
      ...
      toast.success('Noise cleanup queued.', {
        description: 'Zemara will create a separate cleaned copy on this device.'
      })
    } catch (error) {
